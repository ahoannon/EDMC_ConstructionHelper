...
class CH_Preferences():
    ...
    def prefs_ui(self, parent: nb.Notebook):
        ...
        self.create_label_entry(frame_overlay, "X-Position (pixels right/left from border):", "overlayX", 0, 1 )
        self.flush_X = tk.IntVar()
        self.flush_x_label = nb.Label(frame_overlay,text="Flush to:")
        ...
        self.flush_y_label = nb.Label(frame_overlay,text="Flush to:")
    ...
    def save_preferences(self):
        ...
        config.set(self.Prefix+"flushRight", bool(self.flush_X.get()))
        config.set(self.Prefix+"overlayY", str(int(self.overlayY_entry.get())))
        config.set(self.Prefix+"flushBottom", bool(self.flush_Y.get()))
